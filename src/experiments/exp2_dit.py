...
residual MLP layers are transferred to trainable quantum mixing operators.
  ...
  Phase 1 — classical training (soft identity anchor keeps cores near-unitary).
  Phase 2 — zero-shot quantum transfer of every core via T(W)=(Q,U_A,H),
            H = i log U_A  (no quantum-side training).
...
# ============================================================================
# Classical residual core + trainable quantum core
# ============================================================================
class ClassicalResLinear(nn.Module):
    ...
    def __init__(self, in_features):
        ...
        self.weight = nn.Parameter(near_identity_weight(in_features))
...
class ConditionalDiTBlock(nn.Module):
    def __init__(self, dim, num_heads):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(dim, dim * 4), nn.GELU(), nn.Linear(dim * 4, dim), self.core_layer,
        )
...
def run(epochs_stage1=80, epochs_stage3=20, timesteps=100, batch_size=128, k=16, save_pdf=False):
    ...
    loader, _ = mnist_loaders(batch_size=batch_size, pad=2, drop_last=True, num_workers=2)
