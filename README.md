...
Experiments IV and V load pre-computed JSON results; the rest run live (quick-demo
parameters by default, full-paper parameters noted as comments).
...
src/
  ...
  experiments/
    validation.py        Val A/B/C: method verification on MNIST
...
result/
  *.png                  output figures (regenerated by tutorial.ipynb)
  result_data/           pre-computed JSON results (Exp III, IV, V)
docs/
...
| **Val C** | Manifold merging | Generator-space blend outperforms weight averaging |
...
| **Exp III** | IBM hardware fidelity | Hellinger F > 0.999, TVD < 0.016 (k = 2 … 16, 1–4 qubits) |
